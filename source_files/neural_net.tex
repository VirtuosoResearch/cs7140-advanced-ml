...
\begin{align*}
    ...
    &= \exarg{\cQ}{\phi(f)} - \log \exarg{\cP}{\exp(\phi(f))}
\end{align*}
Thus,
\begin{align*}
    \log\exarg{\cP}{\exp(\phi(f))}
    &= \exarg{\cP}{\phi(f)} - D_{KL}(\cQ || \cP) + D_{KL}(\cQ || \cP_G) \\
...
This gives
\begin{align*}
    \exarg{\cP}{\exp(2(n-1) \Delta^2(f))}
    \ge \exp\left( 2(n-1) \exarg{\cQ}{\Delta^2(f)} - D_{KL}(\cQ || \cP)\right)
\end{align*}
Let us take the expectation over the training samples on both sides and notice that the prior $\cP$ does not depend on the training samples.
\begin{align*}
    \exarg{S}{\exarg{\cP}{\exp(2(n-1) \Delta^2(f))}}
    \ge \exarg{S}{\exp\left( 2(n-1) \exarg{\cQ}{\Delta^2(f)} - D_{KL}(\cQ || \cP)\right)}
\end{align*}
For a fixed $f$, we can bound $\Delta(f)$ using Hoeffding:
\[ \Pr[\abs{\Delta(f)} \ge \epsilon] \le 2\exp(-2n \epsilon^2) \]
Therefore, we have
\begin{align*}
    \exarg{S}{\exp(2(n-1) \Delta^2(f))}
    &= \int_{0}^{\infty} \Pr[\exp(2(n-1) \Delta^2(f) \ge s)] ds \\
    &\le 1 + \int_1^{\infty} \Pr[\abs{\Delta(f)} \ge \sqrt{log(s) / (2(n-1)}] ds \\
    &\le 1 + \int_1^{\infty} 2\exp(-2n log(s) / (2(n-1)))  ds \\
    & = 1 + 2(n-1) \le 2n
\end{align*}
Then by Markov's inequality,
\begin{align*}
    \Pr[\exp(2(n-1) \exarg{\cQ}{\Delta^2(f)} - D_{KL}(\cQ || \cP)) \ge 2n / \delta]
    \le \delta
\end{align*}
That is, with probability at least $1 - \delta$,
\[ 2(n-1) \exarg{\cQ}{\Delta^2(f)} \le \log(2n / \delta) + D_{KL}(\cQ || \cP) \]
Using Jensen's inequality, the left is greater than $\exarg{\cQ}{\Delta(f)}^2$.
Taking square root on both sides gives us the McAllester's bound.

\paragraph{PAC-Bayessian bound (Catoni):}
Another version of the PAC-Bayesian theorem is shown by Catoni \citep{catoni2007pac,mcallester2013pac,alquier2021user}.
...
{\bf Suggested readings:} \cite{ju2023generalization}.
