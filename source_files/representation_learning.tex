...
In many modern ML settings, one would like to use a large, often pretrained model, to solve a downstream task.
...
We assume a linear model specificed by an unknown parameter vector $\beta^{(i)} \in \real^p$:
\begin{align*}
...
We refer to the first task as the \emph{source} and the second task as the \emph{target} for ease of presentation.
We assume $n_2 > p$ while $n_1$ can be either less or greater than $p$.

We study random designs where the covariates are given by
...
Then, we minimize the following objective, parameterized by a shared variable $B \in \real^{p}$:
...
We can now compare the above bias-variance to that of single-task learning.
That would just be \[ \sigma^{2} \tr\big[\Sigma^{(2)} ( (X^{(2)})^{\top} X^{(2)} )^{-1} \big], \]
which is the bias of the ordinary least squares (OLS) estimator for the target task.
